	...
	<div class="row text-center">
		...
		<div class="col-md-9">
			...
			<a class="btn btn-sm btn-info pull-right" href="{{ URL::to('funcionario/create') }}"><i class="glyphicon glyphicon-plus"></i> Novo</a>
			<br/>
				...
				<table class="table table-striped table-hover table-list-search">
					...
					@foreach($funcionario as $key => $value)
						<tr>
							<td> {{ $value->id}} </td>
							<td> {{ $value->nome}} </td>
							<td class="phone"> {{ $value->telefone}} </td>
							<td> {{ $value->masp}} </td>
							...
							<td width="300px">
								...
								<!-- delete the funcionario (uses the destroy method DESTROY /funcionario/{id} -->
								<!-- we will add this later since its a little more complicated than the other two buttons -->

								{{ Form::open(array('url' => 'funcionario/' . $value->id, 'class' => 'pull-right')) }}
								...
								<a class="btn btn-sm btn-success pull-right" href="{{ URL::to('funcionario/' . $value->id) }}"><i class="glyphicon glyphicon-eye-open"></i></a> 

								<!-- edit this funcionario (uses the edit method found at GET /funcionario/{id}/edit -->
								<a class="btn btn-sm btn-info pull-right" href="{{ URL::to('funcionario/' . $value->id . '/edit') }}"><i class="glyphicon glyphicon-pencil"></i></a>
