...
<div class="container">
	<div class="row">
		...
		<div class="col-md-9">
			...
			<a class="btn btn-sm btn-info pull-right" href="{{ URL::to('genero/create') }}"><i class="glyphicon glyphicon-plus"></i> Novo</a>
			<br/>
					...
					@foreach($genero as $key => $value)
						<tr>
							...
							<td width="300px">
								...
								<!-- delete the genero (uses the destroy method DESTROY /genero/{id} -->
								<!-- we will add this later since its a little more complicated than the other two buttons -->

								{{ Form::open(array('url' => 'genero/' . $value->id, 'class' => 'pull-right')) }}
									{{ Form::hidden('_method', 'DELETE') }}
									{{ Form::submit('x', array('class' => 'btn btn-sm btn-danger')) }}
								{{ Form::close() }} 

								<!-- show the genero (uses the show method found at GET /genero/{id} -->
								<a class="btn btn-sm btn-success pull-right" href="{{ URL::to('genero/' . $value->id) }}"><i class="glyphicon glyphicon-eye-open"></i></a> 

								<!-- edit this genero (uses the edit method found at GET /genero/{id}/edit -->
								<a class="btn btn-sm btn-info pull-right" href="{{ URL::to('genero/' . $value->id . '/edit') }}"><i class="glyphicon glyphicon-pencil"></i></a>                              
