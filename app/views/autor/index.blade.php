...
<div class="container">
	<div class="row">
		...
		<div class="col-md-9">
			...
			<a class="btn btn-sm btn-info pull-right" href="{{ URL::to('autor/create') }}"><i class="glyphicon glyphicon-plus"></i> Novo</a>
			<br/>
					...
					@foreach($autor as $key => $value)
						<tr>
							...
							<td width="300px">
								...
								<!-- delete the autor (uses the destroy method DESTROY /autor/{id} -->
								<!-- we will add this later since its a little more complicated than the other two buttons -->

								{{ Form::open(array('url' => 'autor/' . $value->id, 'class' => 'pull-right')) }}
									{{ Form::hidden('_method', 'DELETE') }}
									{{ Form::submit('x', array('class' => 'btn btn-sm btn-danger')) }}
								{{ Form::close() }} 

								<!-- show the autor (uses the show method found at GET /autor/{id} -->
								<a class="btn btn-sm btn-success pull-right" href="{{ URL::to('autor/' . $value->id) }}"><i class="glyphicon glyphicon-eye-open"></i></a> 

								<!-- edit this autor (uses the edit method found at GET /autor/{id}/edit -->
								<a class="btn btn-sm btn-info pull-right" href="{{ URL::to('autor/' . $value->id . '/edit') }}"><i class="glyphicon glyphicon-pencil"></i></a>
			...
			<smalll>Autores Cadastrados:</small> <b>{{ $quant }}</b>
